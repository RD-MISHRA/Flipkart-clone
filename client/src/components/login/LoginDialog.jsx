...
import { Dialog, Box, TextField, Typography, Button, styled, Checkbox, FormControlLabel } from '@mui/material';
import { useState, useContext } from 'react';
import { toast } from 'react-toastify';
import { DataContext } from '../../context/DataProvider.jsx';
import { authenticateSignup, authenticateLogin } from '../../service/api.js';
...
const LoginDialog = ({ open, setOpen }) => {
  ...
  const loginUser = async () => {
    ...
    if (response.status === 200) {
      ...
      setError(true);
      toast.error("Cannot login. Check your ID and password.");
    }
  };

  const handleGuestLogin = (e) => {
    setGuestLogin(e.target.checked);
    if (e.target.checked) {
      setLogin({
        username: 'guest',
        password: '123456'
    ...
    <Dialog open={open} onClose={handleClose} PaperProps={{ sx: { maxWidth: 'UNSET' } }}>
      <Component>
        <Box style={{ display: 'flex', height: '100%' }}>
          ...
          {account.view === 'login' ?
            <Wrapper>
              <TextField variant="standard" onChange={(e) => onValueChange(e)} name='username' label="Enter Email/Mobile number" value={login.username} />
              {error && <Error>Please enter valid username or password</Error>}
              <TextField variant="standard" onChange={(e) => onValueChange(e)} name='password' label="Password" type="password" value={login.password} />
              <FormControlLabel
                control={<Checkbox checked={guestLogin} onChange={handleGuestLogin} />}
